...
from models.MobileNet_IQA import MoNet as MobileNet
import argparse
...
def inference(config):
    ...
    if model_type == 'mobileIQA':
        ...
        model = Model(drop=0.1, dim_mlp=768).to(device)
    ...
    with torch.inference_mode():
        for img_tensor, img_name in tqdm(dataloader):
            ...
            resize_img = resize_img.to(device)
...
if __name__ == '__main__':
    ...
    for image_name, score in iq_score:
        print(f"Image: {image_name}, Score: {score:.4f}")
