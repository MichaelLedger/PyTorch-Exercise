...
class Self_Attention(nn.Module):
    def __init__(self, in_dim):
        ...
        self.qConv = nn.Conv2d(in_channels=in_dim, out_channels=19, kernel_size=1)  # Optimized size
        self.kConv = nn.Conv2d(in_channels=in_dim, out_channels=19, kernel_size=1)  # Optimized size
        self.vConv = nn.Conv2d(in_channels=in_dim, out_channels=153, kernel_size=1)  # Optimized size
...
class Local_Distortion_Aware(nn.Module):
    def __init__(self, in_dim, out_dim):
        super().__init__()
        self.cnn1 = nn.Conv2d(in_dim, 307, 1, 1)  # Optimized size
        self.grelu = nn.GELU()
        self.avg2 = nn.AvgPool2d(4, 4)
        self.cnn2 = nn.Conv2d(307, 153, 1, 1)  # Optimized size
...
class MoNet(nn.Module):
    def __init__(self, drop=0.1, dim_mlp=768, img_size=224):
        ...
        # Optimized LDA layers
        self.LDA1 = Local_Distortion_Aware(9, 153)  # Optimized size
        self.LDA2 = Local_Distortion_Aware(14, 153)  # Optimized size
        self.LDA3 = Local_Distortion_Aware(24, 153)  # Optimized size
        self.LDA4 = Local_Distortion_Aware(67, 153)  # Optimized size
        self.LDA5 = Local_Distortion_Aware(576, 153)  # Optimized size
        
        self.MALs = nn.ModuleList()
        for _ in range(3):
            self.MALs.append(MAL(153, feature_num=5, feature_size=22))  # Optimized size
            
        self.fusion_mal = MAL(153, feature_num=3, feature_size=22)  # Optimized size

        self.cnn = nn.Sequential(
            nn.Conv2d(153, 76, 1),  # Optimized size
            nn.ReLU(inplace=True),
            nn.AvgPool2d((2, 2)),
            nn.Conv2d(76, 76, 3),  # Optimized size
            nn.ReLU(inplace=True),
            nn.AdaptiveAvgPool2d(1),
        )
        self.fc_score = nn.Sequential(
            nn.Linear(76, 38),  # Optimized size
            nn.ReLU(),
            nn.Dropout(drop),
            nn.Linear(38, 1),
            nn.Sigmoid()
    ...
    def forward(self, full_img, teacher_model=None):
        ...
        for idx, _ in enumerate(local_features):
            if feature_list is None:
                feature_list = getattr(self, 'LDA{}'.format(idx + 1))(local_features[idx])
            else:
